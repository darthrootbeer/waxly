...
def validate_term(term_data: Dict[str, Any], schema: Dict[str, Any], term_file: Path) -> List[str]:
...
def validate_custom_rules(term_data: Dict[str, Any], term_file: Path) -> List[str]:
    ...
    if term_data.get('slug') != expected_slug:
        errors.append(f"Slug '{term_data.get('slug')}' doesn't match filename '{expected_slug}'")
...
@click.option('--schema-path', default='schema/term.schema.json', help='Path to term schema')
